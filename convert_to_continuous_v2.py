...
# -*- coding: utf-8 -*-
"""
Marp Markdown to Continuous Scroll HTML Converter (v2)
마크다운 슬라이드를 연속 스크롤 HTML로 변환하는 스크립트

markdown2 라이브러리를 사용하여 더 정확한 마크다운 렌더링을 제공합니다.
...
사용법:
    단일 파일 변환:
        python convert_to_continuous_v2.py <input_markdown_file> [output_html_file]
        
    폴더 일괄 변환:
        python convert_to_continuous_v2.py <input_directory> [output_directory]

예시:
    # 단일 파일 변환 (자동 출력 파일명)
    python convert_to_continuous_v2.py slides_part1_reorganized.md
    
    # 단일 파일 변환 (출력 파일명 지정)
    python convert_to_continuous_v2.py slides_part1_reorganized.md my_slides.html
    
    # 폴더 내 모든 .md 파일 변환 (html_output 폴더에 저장)
    python convert_to_continuous_v2.py marp_md/
    
    # 폴더 내 모든 .md 파일 변환 (지정된 출력 폴더에 저장)
    python convert_to_continuous_v2.py marp_md/ html/
"""
...
def markdown_to_html(text):
    ...
    # 코드 블록에 대한 추가 스타일링
    # <pre><code> 태그를 찾아서 스타일 개선
    html = re.sub(r'<pre><code class="language-(\w+)">', r'<pre class="language-\1"><code>', html)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("사용법:")
        print("  단일 파일: python convert_to_continuous_v2.py <input_markdown_file> [output_html_file]")
        print("  폴더 처리: python convert_to_continuous_v2.py <input_directory> [output_directory]")
        print()
        print("예시:")
        print("  python convert_to_continuous_v2.py slides_part1_reorganized.md")
        print("  python convert_to_continuous_v2.py marp_md/")
        print("  python convert_to_continuous_v2.py marp_md/ html/")
        return
